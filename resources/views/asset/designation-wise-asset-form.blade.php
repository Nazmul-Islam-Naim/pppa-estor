...
@section('title', 'Designation Wise Asset Form')
@section('content')
            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Designation Wise Permission Form</div>
                    </div>
                    {!! Form::open(array('route' =>['assign-asset.store'],'method'=>'POST','files'=>true)) !!}
                    <div class="card-body">
                      <div class="col-md-12">
                                ...
                                <?php $row_num = 1; ?>
                                    <tr id="row_{{$row_num}}">
                                        <td style="border: 1px solid #fff; width:40%">
                                            <select name="addmore[{{$row_num}}][asset_id]" id="item" onchange="typeAndSubType(this.value, <?php echo $row_num?>)" class="form-control select-single js-states select2" data-live-search="true" required="">
                                        ...
                                        <td style="border: 1px solid #fff">
                                            <input type="number" step="any" class="form-control max_limit_{{$row_num}}"
                                                name="addmore[{{$row_num}}][max_limit]" id="max_limit" required="" autocomplete="off">
                                        </td>
                                        <td style="border: 1px solid #fff">
                                            <textarea name="addmore[{{$row_num}}][des]" class="form-control des_{{$row_num}}" id="des" style="height:40px" autocomplete="off"></textarea>
                                        </td>
                                        <td>
...
$(document).ready(function() {
    ...
    $("#addone").on('click', function() {
        ...
        row += '<td>';
        row += ' <select name="addmore['+i+'][asset_id]" id="item" onchange="typeAndSubType(this.value, '+i+')" class="form-control select2" required="">';
        ...
        row += '</td>';
        row += '<td>';
        row += ' <input type="number" step="any" class="form-control max_limit_' + i +'" name="addmore[' + i +'][max_limit]" id="max_limit" required="" autocomplete="off">';
        row += '</td>';
        row += '<td>';
