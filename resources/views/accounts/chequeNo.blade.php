            ...
            <div class="field-wrapper">
              <div class="input-group">
                <select name="cheque_book" class="form-control select2" required> 
                  ...
                  <option value="{{encrypt($chequebook->id)}}">{{$chequebook->name}}</option>
                  @endforeach
                </select>
              </div>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="actions" style="height: 25px">
                        ...
                        {{Form::open(array('route'=>['cheque-no.destroy',encrypt($data->id)],'method'=>'DELETE'))}}
                      ...
                      <div class="modal fade" id="editModal{{$data->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                          ...
                          <div class="modal-content">
                            ...
                            {!! Form::open(array('route' =>['cheque-no.update', encrypt($data->id)],'method'=>'PUT')) !!}
                                  ...
                                  <div class="field-wrapper">
                                    <div class="input-group">
                                      <select name="cheque_book" class="form-control select2" required> 
                                        <option value="">Select</option>
                                        @foreach($allchequebook as $chequebook)
                                        <option value="{{encrypt($chequebook->id)}}" {{(($chequebook->id)==($data->cheque_book))? 'selected':''}}>{{$chequebook->name}}</option>
                                        @endforeach
                                      </select>
                                    </div>
